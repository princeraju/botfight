...
void printToTrainFile(FILE *fp2){
    int i,j;
    for(i=0;i<SIZE;i++){
        for(j=0;j<SIZE;j++){
            if(j!=0)
                fprintf(fp2,",");
            if( (board[i][j]&BOTH)==BOTH )
                fprintf(fp2,"1,1");
            else if( (board[i][j]&RIGHT)==RIGHT )
                fprintf(fp2,"1,0");
            else if( (board[i][j]&DOWN)==DOWN )
                fprintf(fp2,"0,1");
            else
                fprintf(fp2,"0,0");
        }
...
int main(int argc, char* argv[])
{
    ...
    while (!done)
    {
        ...
        {
            //init();
            playerId = input[6] - '1';  //hack to get player id
        ...
        else if (commandis(input, "YOUR_MOVE"))
        {
            ...
            levels=3;//get_level_dynamic(available_moves);
            ...
            if(mymove.move == RIGHT)
                os << "("<<(int)mymove.x <<"," <<(int)mymove.y<<"),("<<(int)mymove.x<<","<<(int)mymove.y+1<<")\n";
            if(mymove.move == DOWN)
                os << "("<<(int)mymove.x <<"," <<(int)mymove.y<<"),("<<(int)mymove.x+1<<","<<(int)mymove.y<<")\n";
            available_moves--;
        }
